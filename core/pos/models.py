...
from core.pos.utilities.xml_functions import generate_access_key
...
class Receipts(models.Model):
    ...
    def get_current_number(self, voucher_type=VOUCHER_TYPE[0][0]):
        number = self.start_number
        search = None
        if voucher_type == VOUCHER_TYPE[0][0]:
            search = self.sale_set.filter().order_by('-id')
        elif voucher_type == VOUCHER_TYPE[2][0]:
            search = self.creditnote_set.filter().order_by('-id')
        if search:
            number = int(search[0].voucher_number)
        return f'{number:09d}'
...
class Sale(models.Model):
    ...
    receipts = models.ForeignKey(Receipts, on_delete=models.PROTECT, verbose_name='Tipo de comprobante')
    ...
    status = models.CharField(max_length=20, choices=VOUCHER_STATUS, default=VOUCHER_STATUS[0][0], verbose_name='Estado')
    ...
    def generate_voucher_number(self):
        number = int(self.receipts.get_current_number()) + 1
        return f'{number:09d}'
    ...
    def generate_pdf_authorized(self):
        ...
        with tempfile.NamedTemporaryFile(delete=True) as file_temp:
            ...
            file_temp.flush()
            self.pdf_authorized.save(name=f'{self.access_code}.pdf', content=File(file_temp))

    def generate_xml(self):
        access_code = generate_access_key(self)
        ...
        ElementTree.SubElement(xml_tax_info, 'codDoc').text = self.receipts.code
        ...
        ElementTree.SubElement(xml_info_invoice, 'identificacionComprador').text = self.client.user.dni
        ...
        ElementTree.SubElement(xml_payment, 'formaPago').text = PAYMENT_METHODS[5][0]
        ElementTree.SubElement(xml_payment, 'total').text = f'{self.total:.2f}'
        ElementTree.SubElement(xml_payment, 'plazo').text = '30'
        ...
        ElementTree.SubElement(xml_additional_info, 'campoAdicional', nombre='dirCliente').text = self.client.address
        ElementTree.SubElement(xml_additional_info, 'campoAdicional', nombre='telfCliente').text = self.client.mobile
        ElementTree.SubElement(xml_additional_info, 'campoAdicional', nombre='Observacion').text = f'FACTURA # {self.voucher_number}'
        return ElementTree.tostring(root, xml_declaration=True, encoding='utf-8').decode('utf-8').replace("'", '"'), access_code

    def toJSON(self):
        item = model_to_dict(self)
        item['company'] = self.company.toJSON()
        item['client'] = self.client.toJSON()
        item['receipts'] = self.receipts.toJSON()
    ...
    def delete_receipts(self):
        queryset = ReceiptStates.objects.filter(receipts_id=self.receipts_id, content_type__model=self.__class__.__name__.lower())
        if queryset.exists():
            for i in queryset.filter(object_id=self.id):
                i.delete()

    def generate_electronic_invoice(self):
        from core.pos.utilities.xml_electronic_signature import XMLElectronicSignature
        if self.status != VOUCHER_STATUS[4][0]:
            xml_electronic_signature = XMLElectronicSignature()
            if self.status == VOUCHER_STATUS[0][0]:
                if not xml_electronic_signature.create_xml(self):
                    return False
            if self.status == VOUCHER_STATUS[1][0]:
                if not xml_electronic_signature.firm_xml(self):
                    return False
            if self.status == VOUCHER_STATUS[2][0]:
                if not xml_electronic_signature.validate(self):
                    return False
            if self.status == VOUCHER_STATUS[3][0]:
                return xml_electronic_signature.authorize(self)
        return True
...
class ReceiptStates(models.Model):
    date_joined = models.DateField(default=datetime.now)
    datetime_joined = models.DateTimeField(default=datetime.now)
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')
    receipts = models.ForeignKey(Receipts, on_delete=models.PROTECT)
    status = models.CharField(max_length=20, choices=VOUCHER_STATUS, default=VOUCHER_STATUS[0][0])
    ...
    def toJSON(self):
        item = model_to_dict(self)
        item['instance'] = self.content_object.toJSON()
        item['receipts'] = self.receipts.toJSON()
        item['status'] = {'id': self.status, 'name': self.get_status_display()}
        item['date_joined'] = self.date_joined.strftime('%Y-%m-%d')
    ...
    class Meta:
        ...
        default_permissions = ()
        permissions = (
            ('view_receipt_states', 'Can view Estado de Comprobante Factura'),
            ('delete_receipt_states', 'Can delete Estado de Comprobante Factura'),
        )
        ordering = ['id']
...
class CreditNote(models.Model):
    ...
    def generate_voucher_number(self):
        number = int(self.receipts.get_current_number(voucher_type=VOUCHER_TYPE[2][0])) + 1
        return f'{number:09d}'
    ...
    def generate_xml(self):
        ...
        ElementTree.SubElement(xml_info_invoice, 'identificacionComprador').text = self.sale.client.user.dni
        ...
        ElementTree.SubElement(xml_info_invoice, 'codDocModificado').text = self.sale.receipts.code
